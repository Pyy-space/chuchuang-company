...
export const useGame = () => {
  ...
  useEffect(() => {
    ...
    socket.on('roundEnded', (state: GameState) => {
      setGameState(state);
    });
  ...
  const drawFromMarket = useCallback((roomId: string, marketIndex: number) => {
    if (!socket) return;
    socket.emit('drawFromMarket', { roomId, marketIndex });
  }, [socket]);

  const playCardToMarket = useCallback((roomId: string, cardIndex: number) => {
    if (!socket) return;
    socket.emit('playCardToMarket', { roomId, cardIndex });
  }, [socket]);

  const playCardToInvestment = useCallback((roomId: string, cardIndex: number) => {
    if (!socket) return;
    socket.emit('playCardToInvestment', { roomId, cardIndex });
  }, [socket]);
